// src/components/SaveToggleButton.tsx
'use client';

import { useEffect, useState } from 'react';
import { Heart, Loader2 } from 'lucide-react';
import { toggleSaved, fetchIsSaved } from '@/lib/queries/savedGamesApi';
...
export default function SaveToggleButton({
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        'rounded-full bg-gray-900 text-white disabled:opacity-60',
    ...
    >
      {loading ? (
        ...
        <Heart
          className="h-5 w-5"
          fill={active ? 'var(--purple)' : 'transparent'}
          stroke={active ? 'var(--purple)' : 'currentColor'}
          strokeWidth={3}
        />
      )}
